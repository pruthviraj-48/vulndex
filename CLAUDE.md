...
**VULNDEX** — a self-contained, offline-capable study console covering **139 web
...
assets/css/style.css    monochrome black/red/white flat pro theme (no 3D/glass) + a11y/map layer
assets/js/app.js        engine — hash router, search, render, progress, attack map (~40K)
assets/js/vulns.js      data — window.VULNS = [...139 dossiers...] (~7.4M, single line)
...
- **Attack Chain Map** (`#/map`, `renderMap()`) — SVG chord diagram of all 139
...
- Flat layered surfaces (solid dark-gray cards, 1px borders, hover = lift + red
  edge); responsive, keyboard-nav, reduced-motion aware. Matrix/scanlines/noise/
  aurora/glitch are disabled in CSS; `bindTilt()` in `app.js` is a no-op (no 3D).
- External deps loaded by CDN only: Google Fonts (Space Grotesk / JetBrains Mono /
  Inter; Orbitron link kept but unused). Everything else is local.

## Current status

- **Type:** static site, complete. 139/139 vuln dossiers present in `vulns.js` (121 from the source list + 12 coverage-gap vulns + 6 modern: LLM prompt injection, XS-Leaks, XSLT injection, cookie tossing, client-side path traversal, HTTP/2 Rapid Reset). **~10,200 Test A→Z steps** (avg ~76/vuln; each step has a distinct `how:{m,t,c}` — repetition kept near-0 by a family-pool ++ generic-pool cycle). Bulk depth generated by a 133-agent research workflow then merged.
